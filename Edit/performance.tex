...
\end{equation}
Here $d^{\text{own}}_{i}$ is an indicator for state-owned banks, capturing institutional differences in cost behavior that may arise from public mandates, government guarantees, or distinct operational incentives; and $d^{\text{seg}}_{i}$ is an indicator for banks specializing in the consumer/retail segment, capturing segment-specific cost structures that differ from those of diversified commercial banks. The exponential link function ensures $\sigma_{\dot{u}}>0$ and allows the scale of inefficiency to vary with bank characteristics while preserving the half-normal distributional assumption. Because our estimation strategy relies on the variance--covariance structure of the composite errors \citep{hou2023}, incorporating determinants through the variance parameterization is the natural route; it modifies the diagonal of the theoretical covariance matrix, as detailed in Section~\ref{sec3:estimation_gmm}.
...
In short, the GMM estimation identifies the network dependence parameter $\rho$ and the variance parameters associated with the underlying structurally independent noise ($\dot{\nu}_{it}$) and inefficiency ($\dot{u}_{it}$) components. This relies on exploiting moment conditions derived from the distributional assumptions on $\dot{\nu}_{it}$ and $\dot{u}_{it}$. In particular, let $S(\rho,W_t)=(I_N - \rho W_t)^{-1}$. Since $\bm{\varepsilon}_t = S(\rho,W_t)(\dot{\bm{\nu}}_t + \dot{\bm{u}}_t)$ and the pre-network components are independent, the second-moment condition based on the variance-covariance structure is:
\begin{align}
\mathbb{V}(\bm{\varepsilon}_t) &= S(\rho,W_t)\left[\sigma^2_{\dot{\nu}} I_N + \left(1-\frac{2}{\pi}\right)\Sigma_{\dot{u},t}\right] (S(\rho,W_t))^\top
\label{eq:var-cov}
\end{align}
where $\Sigma_{\dot{u},t}$ encodes the inefficiency variance structure. Under the homogeneous specification (M1, M2), $\Sigma_{\dot{u},t} = \sigma^2_{\dot{u}} I_N$, reducing the bracketed term to the scalar $[\sigma^2_{\dot{\nu}} + (1-2/\pi)\sigma^2_{\dot{u}}]I_N$. Under the heterogeneous specification (M3), $\Sigma_{\dot{u},t} = \mathrm{Diag}\!\big(\sigma^2_{\dot{u}}(\bm{d}_{1};\bm{\delta}),\dots,\sigma^2_{\dot{u}}(\bm{d}_{N};\bm{\delta})\big)$, where each diagonal entry is determined by the bank's characteristics through the exponential link in equation~\eqref{eq:het_inefficiency}.
...
where $\mu^{*}_{it} = \dot{\varepsilon}_{it}{\sigma}^2_{\dot{u},i}/(\sigma^2_{\dot{u},i}+\sigma^2_{\dot{\nu}})$ and $\sigma_{*,i} = \sqrt{\sigma^2_{\dot{u},i}\sigma^2_{\dot{\nu}} / (\sigma^2_{\dot{u},i}+\sigma^2_{\dot{\nu}})}$, using the estimated variance parameters. In the homogeneous case, $\sigma^2_{\dot{u},i} = \sigma^2_{\dot{u}}$ for all $i$; in the heterogeneous case, $\sigma^2_{\dot{u},i} = \sigma^2_{\dot{u}}(\bm{d}_i;\bm{\delta})$ varies by bank.
