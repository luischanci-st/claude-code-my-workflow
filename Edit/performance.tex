...
\date{\small (This version: January 2026. Comments are welcome.)}
...
\begin{abstract}
Financial networks are typically studied for their role in propagating shocks, yet much less is known about how these linkages shape banks' routine operational performance. This paper examines whether interconnectedness in the interbank lending market affects cost efficiency. Using confidential administrative data from the Chilean banking sector (\inpmod{firstyr}–\inpmod{lastyr}), we map the ``physical'' network of bilateral exposures and estimate a network-augmented stochastic cost frontier through a two-step GMM framework adapted to the multi-output structure of banking. We find that interconnectedness is a statistically significant determinant of performance. Specifically, we estimate a negative network dependence parameter, suggesting that stronger network ties are associated with higher cost efficiency in the Chilean market. Decomposing total inefficiency, we find that network effects account for a median reduction of approximately \inpmod{p50EG} percentage points in cost inefficiency relative to the idiosyncratic component. These results provide novel empirical evidence suggesting that interbank networks influence not only the propagation of financial shocks but also the underlying operational performance of banks. Therefore, our findings highlight the importance of monitoring the structure of financial networks as a key element of both stability analysis and performance evaluation.
...
To overcome these empirical challenges, we leverage a unique administrative dataset from the Chilean banking sector alongside advanced efficiency modeling techniques. The data, obtained directly from the financial market regulator\footnote{The \emph{Comisión para el Mercado Financiero} (CMF).}, include detailed transaction records from the interbank lending market (Form C-18) linked with monthly balance sheets and other bank-level controls for nearly all institutions operating in the country between \inpmod{firstyrm} and \inpmod{lastyrm}. This rich panel, based on observed bilateral lending relationships rather than correlation-based proxies, enables us to construct network adjacency matrices that directly capture the intensity and direction of interbank funding ties. Because these matrices are built from realized exposures, they provide a structural measure of financial interconnectedness based on actual balance-sheet linkages.

While these interbank lending relationships represent a central dimension of financial interdependence, they may not necessarly exhaust all potential channels through which banks' performance co-moves. Thus, as a robutness excercise, we complement this exposure-based network with an alternative adjacency matrix inferred from balance-sheet co-movement using recent econometric approaches that identify latent dependence structures. This complementary exercise allows us to distinguish the role of contractual funding ties from broader forms of interdependence, while maintaining our primary focus on how observed interconnectedness shapes cost efficiency.%Check whether I should cite De Paula et al. here
...
%%% --- OLD Section 2.1 (commented out 2026-02-10) ---
...
%\begin{equation}
%    \ln{\mathrm{Cost}}_{it} = \alpha_t + TL(\bm{y}_{it}, \bm{p}_{it}, r_{it}, \bm{z}_{it}; \bm{\beta} ) + \varepsilon_{it},
%\end{equation}
...
%[...interconnectedness paragraphs kept mostly intact in new version...]
...
%%% --- NEW Section 2.1 (rewritten 2026-02-10) ---

\paragraph{The cost function}
We specify a translog (TL) stochastic cost frontier for bank $i=1,\dots,N$ in time period $t=1,\dots,T$:
\begin{equation}
    \ln{\mathrm{Cost}}_{it} = \alpha_t + TL(\bm{y}_{it}, \bm{p}_{it}, r_{it}, \bm{z}_{it}; \bm{\beta} ) + \varepsilon_{it}, \label{eq:model_part1}
\end{equation}
where $\mathrm{Cost}_{it}$ denotes variable cost; $\alpha_t$ is a time-fixed effect capturing macroeconomic shocks and common shifts in the cost frontier; and $TL(\cdot)$ represents the translog functional form in outputs $\bm{y}$ (loan categories), input prices $\bm{p}$ (labor, physical capital, and deposits), quasi-fixed input $\bm{z}$ (equity capital), and a risk measure $r$ (capital adequacy ratio). The parameter vector $\bm{\beta}$ collects the technology coefficients. Linear homogeneity in input prices is imposed by normalizing both the dependent variable and the input prices by the price of one input ($p_{1it}$). Equity capital, given its limited within-bank time variation, also serves as a proxy for bank-specific fixed effects, thereby mitigating unobserved heterogeneity that might otherwise confound the efficiency estimates.

Including a risk measure in the cost function serves two purposes. First, it is standard practice in the banking cost frontier literature: risk-taking affects the cost structure because banks that assume greater risk may incur higher monitoring, provisioning, and funding costs \citep{hughes2013,hughes2019scale,mamonov2024}. Second, and critically for our setting, the risk control strengthens identification of the network parameter. Because our measure of interconnectedness is derived from interbank debt exposures, banks with higher risk appetites may systematically borrow more in interbank markets, generating a correlation between the adjacency matrix and unobserved cost determinants. In the terminology of \citet{manski1993}, this constitutes a contextual effect that could confound the estimated endogenous peer effect. Conditioning on risk within the cost function mitigates this sorting concern by absorbing a key dimension along which banks select into network positions.

\paragraph{Interconnectedness and efficiency}
Following the standard stochastic frontier literature, the error term $\varepsilon_{it}$ in equation \eqref{eq:model_part1} is decomposed into a symmetric noise term $\nu_{it}$ and a non-negative inefficiency term $u_{it}$ \citep{kumbhakar2000,kumbhakar2015}. We extend this framework by explicitly modeling network dependence among banks. Specifically, adopting the approach of \citet{hou2023}, we specify the vector of composite errors $\bm{\varepsilon}_{t} = (\varepsilon_{1t}, \dots, \varepsilon_{Nt})'$ as a function of the performance of connected peers, governed by an $N \times N$ adjacency matrix $W_t$. The structural equation is:
\begin{equation}
\begin{aligned}
...
\end{equation}
where $\rho$ is the network dependence parameter to be estimated. The term $W_t \bm{\varepsilon}_{t}$, often expressed component-wise as $\sum_j w_{ijt} \varepsilon_{jt}$ and defined as the endogenous network lag, represents a weighted combination of the composite errors of other banks to which bank $i$ is financially connected. The element $w_{ijt}$ denotes the predetermined weight assigned to the influence of bank $j$ on bank $i$ at time $t$. This general specification accommodates potentially evolving network structures, with the standard normalization that $w_{iit} = 0$. Solving equation~\eqref{eq:model_part2} for $\bm{\varepsilon}_t$ yields the reduced form:
\begin{equation}\label{eq:reduced_form}
\bm{\varepsilon}_t = S(\rho, W_t)\,\dot{\bm{\varepsilon}}_t, \qquad S(\rho, W_t) \equiv (I_N - \rho W_t)^{-1},
\end{equation}
where $S(\rho, W_t)$ is the spatial multiplier matrix. The matrix $S$ is well-defined provided $|\rho| < 1/\max_t\{\bar{\lambda}(W_t)\}$, where $\bar{\lambda}(W_t)$ denotes the spectral radius of $W_t$. Each element $s_{ij,t}$ of $S$ captures the total (direct and indirect) effect of bank $j$'s pre-network error on bank $i$'s observed composite error.
...
\paragraph{Distributional assumptions and determinants of inefficiency}
Consistent with the standard stochastic frontier literature \citep{kumbhakar2000,kumbhakar2015}, we assume that the underlying (pre-network) error components are independently distributed. The idiosyncratic noise follows a normal distribution, $\dot{\nu}_{it}\sim \mathcal{N}(0,\sigma^2_{\dot{\nu}})$. For the pre-network inefficiency term $\dot{u}_{it}$, we consider two specifications:

\textit{Homogeneous inefficiency.} Under the baseline specification, the inefficiency term follows a half-normal distribution with a common scale parameter:
$$
\dot{u}_{it}\sim \mathcal{N}^+(0,\,\sigma^2_{\dot{u}}).
$$
This imposes that all banks draw inefficiency from the same distribution, so that observed differences in cost performance arise solely from random realizations and the network multiplier.

\textit{Heterogeneous inefficiency.} Following \citet{hou2023} (Section~3.3) and \citet{mamonov2024}, we also consider a specification in which the scale of the inefficiency distribution varies across banks as a function of observable characteristics:
\begin{equation}\label{eq:het_inefficiency}
\dot{u}_{it}\sim \mathcal{N}^+\!\Big(0,\;\sigma^2_{\dot{u}}(\bm{d}_{i};\bm{\delta})\Big), \quad \text{where} \quad \sigma_{\dot{u}}(\bm{d}_{i};\bm{\delta})=\exp\!\big(\delta_0+\delta_1 d^{\text{own}}_{i}+\delta_2 d^{\text{seg}}_{i}\big).
\end{equation}
Here $d^{\text{own}}_{i}$ is an indicator for state-owned banks, capturing institutional differences in cost behavior that may arise from public mandates, government guarantees, or distinct operational incentives; and $d^{\text{seg}}_{i}$ is an indicator for banks specializing in the consumer/retail segment, capturing segment-specific cost structures that differ from those of diversified commercial banks. The exponential link function ensures $\sigma_{\dot{u}}>0$ and allows the scale of inefficiency to vary with bank characteristics while preserving the half-normal distributional assumption \citep{caudill1995}. This variance parameterization differs from the mean-based approach of \citet{battese1995}, in which determinants enter through the conditional mean of a truncated normal. Because our estimation strategy relies on the variance--covariance structure of the composite errors \citep{hou2023}, incorporating determinants through the variance is the natural route: each $\sigma^2_{\dot{u}}(\bm{d}_i;\bm{\delta})$ enters the diagonal of the theoretical covariance matrix directly, making the determinant effects identifiable from the GMM moment conditions without additional distributional complexity (see Section~\ref{sec3:estimation_gmm}).

\paragraph{Nesting structure and model variants}
The specification above nests three models of increasing generality:
\begin{itemize}
    \item[\textbf{M1.}] \textit{Basic spatial cost frontier.} The cost function excludes the risk regressor ($r_{it}$) and inefficiency is homogeneous ($\sigma_{\dot{u}}$ is a scalar). The second-stage parameter vector is $\bm{\theta}=(\rho,\,\sigma^2_{\dot{\nu}},\,\sigma^2_{\dot{u}})$.
    \item[\textbf{M2.}] \textit{With risk control.} The risk measure $r_{it}$ enters the translog cost function; inefficiency remains homogeneous. This specification tests whether controlling for risk-taking alters the estimated network dependence.
    \item[\textbf{M3.}] \textit{With risk and heterogeneous inefficiency.} Risk is included and the inefficiency variance is parameterized as in equation \eqref{eq:het_inefficiency}, so that $\bm{\theta}=(\rho,\,\sigma^2_{\dot{\nu}},\,\bm{\delta})$. This is the most general specification; it allows both the cost frontier and the inefficiency distribution to differ across bank types.
\end{itemize}
Estimating all three models allows us to assess robustness and to quantify the incremental contribution of the risk control and of heterogeneous inefficiency to the network dependence estimate $\hat{\rho}$.
...
\begin{align}
\ln{\mathrm{Cost}}_{it} &= \alpha_{t}^* + TL(\bm{y}_{it}, \bm{p}_{it}, r_{it}, \bm{z}_{it}; \bm{\beta}^*) + \varepsilon^*_{it}, \label{eq:model_step1}
\end{align}
...
where the parameter vector $\bm{\beta}^*$ now excludes the original intercept $\beta_0$; the new time-specific intercept absorbs mean inefficiency, $\alpha_{t}^* = \beta_0 + \alpha_t + \mathbb{E}[u_{it}]$; and the transformed error term is $\varepsilon^*_{it} = \nu_{it} + u_{it} - \mathbb{E}[u_{it}]$. Therefore, by construction, $\mathbb{E}[\varepsilon_{it}^*]=0$, and the resulting model belongs to the family of panel data models (i.e., a panel cost function featuring time-specific effects, where time-invariant individual heterogeneity is primarily accounted for by including quasi-fixed inputs, rather than via separate individual-specific intercepts). Under the homogeneous specification (M1, M2), $\mathbb{E}[\dot{u}_{it}] = \sqrt{2/\pi}\,\sigma_{\dot{u}}$ is common across banks, and when $W_t$ is row-normalized, $\mathbb{E}[u_{it}]$ is likewise identical for all $i$ within each $t$, so that $\alpha_t^*$ is a pure time effect. Under the heterogeneous specification (M3), $\mathbb{E}[\dot{u}_{it}] = \sqrt{2/\pi}\,\sigma_{\dot{u},i}$ varies across banks, introducing a bank-specific component into $\alpha_t^*$. This component is partially absorbed by the quasi-fixed input (equity) that proxies for bank-level heterogeneity; any residual variation is retained in the pseudo-residuals and exploited by the GMM in the second stage.
...
$$
\ln{\mathrm{Cost}}_{it} - TL(\bm{y}_{it}, \bm{p}_{it}, r_{it}, \bm{z}_{it}; \bm{\beta}^*) = \beta_0 + \alpha_t + \nu_{it} + u_{it}
$$
...
e_{it} = \ln{\mathrm{Cost}}_{it} - TL(\bm{y}_{it}, \bm{p}_{it}, r_{it}, \bm{z}_{it}; \bm{\hat{\beta}}^*)
\end{equation}
...
In short, the GMM estimation identifies the network dependence parameter $\rho$ and the variance parameters associated with the underlying structurally independent noise ($\dot{\nu}_{it}$) and inefficiency ($\dot{u}_{it}$) components. This relies on exploiting moment conditions derived from the distributional assumptions on $\dot{\nu}_{it}$ and $\dot{u}_{it}$. Since the pre-network components are mutually independent and cross-sectionally independent, the variance--covariance matrix of the pre-network composite error $\dot{\bm{\varepsilon}}_t = \dot{\bm{\nu}}_t + \dot{\bm{u}}_t$ is:
\begin{equation}\label{eq:var_pre_network}
\mathbb{V}(\dot{\bm{\varepsilon}}_t) = \sigma^2_{\dot{\nu}} I_N + \left(1-\frac{2}{\pi}\right)\Sigma_{\dot{u}}
\end{equation}
where the factor $(1-2/\pi)$ arises because $\mathrm{Var}(\dot{u}_{it}) = (1-2/\pi)\sigma^2_{\dot{u},i}$ for a half-normal random variable with scale $\sigma_{\dot{u},i}$. The matrix $\Sigma_{\dot{u}}$ encodes the inefficiency variance structure. Under the homogeneous specification (M1, M2), $\Sigma_{\dot{u}} = \sigma^2_{\dot{u}} I_N$, reducing equation~\eqref{eq:var_pre_network} to the scalar $[\sigma^2_{\dot{\nu}} + (1-2/\pi)\sigma^2_{\dot{u}}]I_N$. Under the heterogeneous specification (M3), $\Sigma_{\dot{u}} = \mathrm{Diag}\!\big(\sigma^2_{\dot{u}}(\bm{d}_{1};\bm{\delta}),\dots,\sigma^2_{\dot{u}}(\bm{d}_{N};\bm{\delta})\big)$, where each diagonal entry is determined by the bank's characteristics through the exponential link in equation~\eqref{eq:het_inefficiency}. Note that, because the determinants $\bm{d}_i$ are time-invariant bank characteristics, $\Sigma_{\dot{u}}$ does not carry a time subscript.

Applying the reduced-form relationship $\bm{\varepsilon}_t = S(\rho,W_t)\dot{\bm{\varepsilon}}_t$ from equation~\eqref{eq:reduced_form}, the variance--covariance matrix of the observed composite error is:
\begin{align}
\mathbb{V}(\bm{\varepsilon}_t) &= S(\rho,W_t)\left[\sigma^2_{\dot{\nu}} I_N + \left(1-\frac{2}{\pi}\right)\Sigma_{\dot{u}}\right] S(\rho,W_t)^\top
\label{eq:var-cov}
\end{align}
...
Furthermore, considering that practical optimization of $Q(\bm{\theta})$ may encounter challenges such as multiple local minima or irregular objective function surfaces, we adopt a improved deviation from HZK regarding the numerical computation of $\bm{\theta}$. Specifically, we enhance numerical stability and efficiency by noting that the estimation must respect the constraint $|\rho|<1$, associated with the invertibility of the $S(\rho, W_t)$ term. We thus implement a grid search strategy, widely used in the spatial econometrics literature \citep[see, e.g.,][]{Elhorst2014,Elhorst2010,pace1997}. This involves a grid search over the permissible range $\rho \in (-1, 1)$. For each candidate value $\rho_j$ from the grid, the GMM objective function is minimized with respect to the remaining parameters---$(\sigma^2_{\dot{\nu}}, \sigma^2_{\dot{u}})$ in the homogeneous case, or $(\sigma^2_{\dot{\nu}}, \bm{\delta})$ in the heterogeneous case---to obtain conditional estimates. The optimal estimate $\hat{\rho}$ is then selected as the value $\rho_j$ that yields the global minimum of the GMM objective function in Equation \eqref{eq:obj_fn} across all tested grid points, along with its corresponding conditional estimates.\footnote{To efficiently locate the minimum, we first employ a coarser grid for $\rho$ (e.g., using 0.1 increments) to find an initial optimum, followed by a finer grid search (e.g., using 0.001 increments) in the neighborhood of that initial value. This vectorized grid search approach, potentially combined with parallel computing techniques, speeds up the estimation within the GMM framework.}

%%
 \subsubsection{Post-Estimation Calculations}\label{sec3:model-post}
Once the GMM estimation yields consistent estimates of $\hat{\rho}$, $\hat{\sigma}^2_{\dot{\nu}}$, and the inefficiency parameters ($\hat{\sigma}^2_{\dot{u}}$ or $\hat{\bm{\delta}}$), further quantities of interest can be derived.

First, we estimate the mean inefficiency component $\mathbb{E}[\bm{u}_{t}]$. Under the homogeneous specification, $\dot{u}_{it}$ follows a half-normal distribution with common scale, so $\mathbb{E}[\dot{\bm{u}}_t] = \sqrt{2/\pi}\, \hat{\sigma}_{\dot{u}} \,\iota_N$, and the estimate incorporating the network multiplier is:
$$
\hat{\mathbb{E}}[\bm{u}_{t}] = \sqrt{\frac{2}{\pi}} \hat{\sigma}_{\dot{u}}\, S(\hat{\rho},W_t)\iota_N
$$
where $\iota_N$ is an $N\times1$ vector of ones. Since in our application the weights matrix $W_t$ is row-normalized for each $t$, $S(\hat{\rho},W_t)\iota_N$ is a constant vector, and mean inefficiency is identical across banks within each period. Under the heterogeneous specification (M3), $\hat{\sigma}_{\dot{u}}$ is replaced by the bank-specific vector $\hat{\bm{\sigma}}_{\dot{u}} = (\hat{\sigma}_{\dot{u}}(\bm{d}_1;\hat{\bm{\delta}}),\dots,\hat{\sigma}_{\dot{u}}(\bm{d}_N;\hat{\bm{\delta}}))'$, so that:
$$
\hat{\mathbb{E}}[\bm{u}_{t}] = \sqrt{\frac{2}{\pi}}\, S(\hat{\rho},W_t)\,\hat{\bm{\sigma}}_{\dot{u}}
$$
and mean inefficiency varies across banks even within the same period.

Second, we recover estimates of the combined constant and time fixed effects, used subsequently in the wild bootstrap procedure (Section~\ref{sec3:bootstrap}). Recalling the estimated time-specific component $\hat{\alpha}_t^* = \bar{e}_t$, which consistently estimates $\alpha_t^* = \beta_0 + \alpha_t + \mathbb{E}[u_{it}]$, one can compute: $(\widehat{\beta_0 + \alpha_t}) = \hat{\alpha}_t^* - \hat{\mathbb{E}}[u_{it}]$.\footnote{Separating the overall constant $\beta_0$ from the individual time effects $\alpha_t$ would require an additional normalization constraint (e.g., $\sum_t \alpha_t = 0$), which is not pursued here.}

Third, and most importantly for the economic analysis, following HZK's adaptation of \cite{jondrow1982} (JLMS), we predict the bank-specific inefficiency component. This term is defined by the conditional expectation of the underlying network-independent $\dot{u}_{it}$ given the underlying network-independent composite error $\dot{\varepsilon}_{it}$:
\begin{align}\label{eq:u_conditional_exp}
\dot{\mu}_{it}=\mathbb{E}[\dot{u}_{it}|\dot{\varepsilon}_{it}] = \mu^{*}_{it} + \sigma_{*,i} \cdot \frac{\phi(m_{it})}{\Phi(m_{it})}
\end{align}

where
\begin{equation}\label{eq:jlms_aux}
\mu^{*}_{it} = \frac{\dot{\varepsilon}_{it}\,\sigma^2_{\dot{u},i}}{\sigma^2_{\dot{u},i}+\sigma^2_{\dot{\nu}}}, \qquad
\sigma_{*,i} = \frac{\sigma_{\dot{u},i}\,\sigma_{\dot{\nu}}}{\sqrt{\sigma^2_{\dot{u},i}+\sigma^2_{\dot{\nu}}}}, \qquad
m_{it} = \frac{\mu^{*}_{it}}{\sigma_{*,i}},
\end{equation}
and $\phi(\cdot)/\Phi(\cdot)$ is the inverse Mills ratio of the standard normal distribution. In the homogeneous case, $\sigma^2_{\dot{u},i} = \sigma^2_{\dot{u}}$ for all $i$, so that $\sigma_{*,i}$ and the ratio $\sigma^2_{\dot{u},i}/(\sigma^2_{\dot{u},i}+\sigma^2_{\dot{\nu}})$ are common across banks; in the heterogeneous case (M3), $\sigma^2_{\dot{u},i} = \sigma^2_{\dot{u}}(\bm{d}_i;\bm{\delta})$ varies by bank through equation~\eqref{eq:het_inefficiency}.
...
%%% Bootstrap
\subsubsection{Computation of Standard Errors via Wild Bootstrap}\label{sec3:bootstrap}

Given the multi-step nature of our GMM estimation procedure for the stochastic cost frontier with interconnectedness, deriving analytical standard errors is complex. Therefore, we employ a wild bootstrap approach to assess the sampling variability of the estimated parameters ($\hat{\rho}$, $\hat{\sigma}_{\dot{\nu}}$, and $\hat{\sigma}_{\dot{u}}$. This method is well-suited for models with complex error structures and potential heteroskedasticity, and its applicability to spatial models has been already discussed \citep[see, e.g., HZK for simulation results in a related GMM framework, and][for wild bootstrap with cross-sectional dependence]{gonccalves2020bootstrapping}.
...
\begin{enumerate}
    ...
    \begin{equation*}
    \ln \bm{Cost}^{(b)}_{t} = \widehat{(\beta_0+\alpha_t)}\bm{\iota}_{N_t} + TL(\bm{y}_{t},\bm{p}_{t},r_{t},\bm{z}_{t};\bm{\hat{\beta}}^{*}) + \left(I_N - \hat{\rho}W_t\right)^{-1}\hat{\dot{\bm{\varepsilon}}}_t^{(b)}
    \end{equation*}
...
\end{enumerate}
This entire process is repeated $B$ times. The standard error for each element of $\bm{\theta}$---i.e., $\rho$, $\sigma_{\dot{\nu}}$, and either $\sigma_{\dot{u}}$ (homogeneous models) or the vector $\bm{\delta}$ (heterogeneous model)---is then calculated as the empirical standard deviation of its $B$ bootstrapped estimates.
...
2. Data: Chilean banking system (2010m1–2018m1).
